...
export class CpmBillingService {
  ...
  // - see AdEngineController.serve). No-ops on most calls; only actually
  // bills once every IMPRESSIONS_PER_CPM_BATCH impressions.
  async recordImpression(event: ImpressionEvent): Promise<void> {
    ...
    const counter = await this.counterStore.increment(
      this.counterKey(event.campaign),
    ...
    if (!zone) {
      this.logger.warn(
        `Skipping CPM batch bill for campaign ${event.campaign}: zone ${event.zone} no longer exists`,
      );
      return;
    }

    // Batch number (not the raw impression count) is the idempotency key -
    // WalletManager.recordCampaignSpend's unique (walletId, type,
    // referenceId) constraint means a retried/duplicated increment for the
    // SAME batch can't double-bill, while each new batch gets its own row.
    const batchNumber = counter.count / IMPRESSIONS_PER_CPM_BATCH;

    await this.adBillingService.billCpmBatch(
      event.campaign,
      zone.publisherId,
      event.maxCpm,
  ...
  private counterKey(campaignId: string): string {
